     ...
     <ul >
       ...
       <li >
         <?php if($designation=="Office stuff"){
             echo '<a  href="hall_office.php" class="active">Applicant</a>';
             }
             else {
...
<div class="container ">
 ...
 <?php
 $counter=0;
 $query="SELECT  * from allot_info WHERE hall_name='$hall_name' and access=1";
 $send=mysqli_query($connection,$query) or die(mysqli_error($connection));
while ($result=mysqli_fetch_array($send)) {
  $counter=$counter+1;
  $allot_name=$result['allot_name'];
  $allot_id=$result['id'];

echo "<tr>";
echo "<td>$counter</td>";
echo "<td>$allot_name</td>";
echo '<td><form class="" action="applicant_list.php" method="post">
  <input type="hidden" name="allot_id" ';
  echo "value='$allot_id'>";
  echo "<input type='submit' class='linkButton' name='s_applicant' value='Show applicant list'>";
echo "</form></td>";
echo "</tr>";
 }
  ?>
